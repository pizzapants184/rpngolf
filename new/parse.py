...
def parse(source):
	...
	#	raise ValueError
		...
		lookup = state.base[source[0].lower()]
		if isinstance(lookup, modules.RGSubModuleBase):
			...
			while True:
				...
				if not isinstance(lookup, modules.RGSubModuleBase):
					return [routines.Routine(source[0].lower()+source[1:depth])]+parse(source[depth:])
		elif isinstance(lookup, functions.RGFunction):
			return [routines.Routine(source[0].lower())]+parse(source[1:])
